...
use crate::entities::character::{CharacterRepository, SpriteGroup, use_characters_refresh};

#[component]
pub fn RenameSpriteGroupButton(character_name: String, sprite_group: SpriteGroup) -> Element {
    let mut show_modal = use_signal(|| false);
    let modal_character_name = character_name.clone();
    let modal_sprite_group = sprite_group.clone();
    ...
    rsx! {
        ...
        if show_modal() {
            RenameSpriteGroupModal {
                character_name: modal_character_name.clone(),
                sprite_group: modal_sprite_group.clone(),
...
fn RenameSpriteGroupModal(
    ...
    let old_name = sprite_group.name.clone();
    ...
    let on_submit = {
        ...
        move |evt: FormEvent| {
            ...
            match repo.rename_sprite_group(&character_name, &old_name, new_name_trimmed) {
                Ok(()) => {
    ...
    rsx! {
        dialog { class: "modal modal-open",
            div { class: "modal-box",
                h3 { class: "text-lg font-bold mb-2", "Sprite Group をリネーム" }
