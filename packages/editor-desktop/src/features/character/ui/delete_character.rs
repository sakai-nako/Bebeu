...
#[component]
pub fn DeleteCharacterButton(name: String) -> Element {
    let mut show_modal = use_signal(|| false);
    let modal_name = name.clone();
...
#[component]
fn DeleteConfirmModal(name: String, onclose: EventHandler<()>) -> Element {
    ...
    let on_confirm = move |_| match repo.delete(&target) {
        Ok(()) => {
            refresh.bump();
            onclose.call(());
            // 削除した Character の詳細ページから一覧へ戻す
    ...
    rsx! {
        dialog { class: "modal modal-open",
            div { class: "modal-box",
                h3 { class: "text-lg font-bold mb-2", "Character を削除" }
                p { class: "py-2",
                    "'"
                    span { class: "font-mono font-semibold", "{name}" }
                    "' を削除しますか？このキャラクターの sprite-groups 等もすべて削除されます。"
                }
