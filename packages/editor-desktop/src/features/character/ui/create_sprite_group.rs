...
#[component]
pub fn CreateSpriteGroupButton(character: Character) -> Element {
...
#[component]
fn CreateSpriteGroupModal(character: Character, onclose: EventHandler<()>) -> Element {
    ...
    let on_submit = move |evt: FormEvent| {
            ...
            .any(|g| g.name == new_name_trimmed)
        {
            error.set(Some(format!(
                "SpriteGroup '{new_name_trimmed}' は既に存在します"
            )));
            return;
        }
        ...
        updated.sprite_groups.push(SpriteGroup {
    ...
    rsx! {
        dialog { class: "modal modal-open",
            div { class: "modal-box",
                h3 { class: "text-lg font-bold mb-4", "新規 Sprite Group" }
